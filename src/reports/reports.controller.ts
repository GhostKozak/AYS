...
export class ReportsController {
  ...
  @ApiQuery({ name: 'limit', type: Number, required: false, description: 'Number of top companies to return (default: 10)' })
  @ApiQuery({ name: 'sortBy', enum: ['tripCount', 'revenue', 'avgTurnaround'], required: false, description: 'Sort criteria (default: tripCount)' })
  @ApiResponse({ status: 200, description: 'Return top companies list' })
  getTopCompanies(@Query() query: ReportQueryDto, @Query('limit') limit?: number, @Query('sortBy') sortBy?: string) {
    return this.reportsService.getTopCompanies(query.period || ReportPeriod.MONTH, limit, sortBy);
  }

  @Get('unload-waiting')
  @ApiOperation({ summary: 'Get stats of vehicles waiting to unload' })
  @ApiQuery({ name: 'period', enum: ReportPeriod, required: false, description: 'Report period (TODAY, MONTH, YEAR, ALL)' })
  @ApiQuery({ name: 'groupBy', enum: ['company', 'vehicle', 'driver'], required: false, description: 'Group results by entity (default: company)' })
  @ApiResponse({ status: 200, description: 'Return waiting stats grouped by entity' })
  getUnloadWaiting(@Query() query: ReportQueryDto, @Query('groupBy') groupBy?: string) {
    return this.reportsService.getUnloadWaitingStats(query.period || ReportPeriod.MONTH, groupBy);
  }
  ...
  @ApiQuery({ name: 'period', enum: ReportPeriod, required: false, description: 'Report period (TODAY, MONTH, YEAR, ALL)' })
  @ApiQuery({ name: 'excludeStatus', type: String, required: false, description: 'Exclude specific status from results' })
  ...
  getAverageTurnaround(@Query() query: ReportQueryDto, @Query('companyId') companyId?: string) {
    return this.reportsService.getAverageTurnaround(query.period || ReportPeriod.MONTH, companyId);
  }
  ...
  @ApiQuery({ name: 'period', enum: ReportPeriod, required: false, description: 'Report period (TODAY, MONTH, YEAR, ALL)' })
  @ApiQuery({ name: 'excludeStatus', type: String, required: false, description: 'Exclude specific status (can be used multiple times)' })
  async exportExcel(@Query() query: ReportQueryDto, @Res() res: Response, @Query('excludeStatus') excludeStatus?: string) {
    res.set({
      'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
      'Content-Disposition': 'attachment; filename=trips-report.xlsx',
    });
    await this.reportsService.exportTripsToExcel(query.period || ReportPeriod.MONTH, res, excludeStatus);
  }
  ...
  @ApiQuery({ name: 'period', enum: ReportPeriod, required: false, description: 'Report period (TODAY, MONTH, YEAR, ALL)' })
  @ApiQuery({ name: 'excludeStatus', type: String, required: false, description: 'Exclude specific status (can be used multiple times)' })
  async exportPdf(@Query() query: ReportQueryDto, @Res() res: Response, @Query('excludeStatus') excludeStatus?: string) {
    res.set({
      'Content-Type': 'application/pdf',
      'Content-Disposition': 'attachment; filename=trips-report.pdf',
    });
    await this.reportsService.exportTripsToPdf(query.period || ReportPeriod.MONTH, res, excludeStatus);
  }
}
